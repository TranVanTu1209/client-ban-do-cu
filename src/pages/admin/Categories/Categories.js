  ...
  return (
    <div>
      <div className='d-flex align-items-center justify-content-between py-2'>
        ...
        <Button variant='contained' color='primary' component={Link} to="/dashboard/category/new">
          Thêm mới
        </Button>
      </div>
      {loading && <ImgLoader />}
          ...
          <TableBody>
            {categoryList.map((c) => (
              <TableRow key={c.id}>
                ...
                <TableCell>
                  <IconButton component={Link} to={`/dashboard/category/edit/${c.id}`}>
                    <EditIcon />
                  </IconButton>
                  <IconButton onClick={() => {
      ...
      { showConfirm && <ConfirmDelete show={showConfirm} close={closeConfirm} clicked={deleteCategoryHandler} /> }
    </div>
  );
};
