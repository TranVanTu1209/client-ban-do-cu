...
const initialState = localStorage.getItem('cartData') ? JSON.parse(localStorage.getItem('cartData')) : {
  cartItems: [],
  giftCode: '',
  subTotal: 0,
  tax: 50
...
const reducer = (state = initialState, action) => {
  ...
  switch (type)
  {
    ...
    case actionTypes.REMOVE_FROM_CART:
      const removedItemIndex = state.cartItems.findIndex(item => item.id === payload);
      ...
      return {
        ...
        subTotal: state.subTotal - Number(removedItem.price) * Number(removedItem.amount)
      }
    case actionTypes.INCREASE_AMOUNT:
      const increasedItem = state.cartItems.find(item => item.id === payload);
      return {
        ...state,
        cartItems: state.cartItems.map(item => item.id === payload ? ({
          ...item,
